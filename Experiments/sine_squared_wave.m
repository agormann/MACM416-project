clear;
clc;
close all;
%% Setup
nx = 1000;
dx = 2*pi/nx;
X = dx*(1:nx);
u0 = sin(X).^2; % sine-squared initial profile
padding = 0.10*(max(u0)-min(u0));
%% Solving
[T,U] = godunov(X,u0,10); % numerical solution
%% Movie plot
f = figure();
f.Position = [100,100,640,640];
v = VideoWriter([pwd '/Movies/sine_squared_wave.mp4'], 'MPEG-4');
v.FrameRate = 60;
open(v);

for i = 1:length(T)
    plot(X, U(i,:), 'r:', 'LineWidth', 2, 'DisplayName', "Sine-squared wave");

    % plot settings
    title(sprintf('t = %.3f', T(i)));
    legend('Location', 'west');
    grid on;
    xlabel('x');
    xticks([0 pi/2 pi 3*pi/2 2*pi]);
    xticklabels({'0','\pi/2','\pi','3\pi/2','2pi'});
    xlim([0,2*pi]);
    ylabel('u(x,t)');
    ylim([min(u0)-padding,max(u0)+padding]);

    % Write current frame to video
    frame = getframe(f);
    writeVideo(v, frame);
    pause(0.01);
end
close(v);
disp('Animation saved as sine_squared_wave.mp4');
%% Plot at specific times for stillframes
times = [0 2 5 10];
indices = ones(1,length(times));
for i = 2:length(times)
    target = times(i);
    [~,index] = min(abs(T-target)); % retrieve index
    indices(i) = index;
end

f = figure();
f.Position = [100,100,360,240];

for i = 1:length(times)
    plot(X, U(indices(i),:), 'r:', 'LineWidth', 2, 'DisplayName', "Sine-squared wave");
    
    % plot settings
    title(sprintf('t = %.3f', times(i)));
    legend('Location', 'northeast');
    grid on;
    xlabel('x');
    xticks([0 pi/2 pi 3*pi/2 2*pi]);
    xticklabels({'0','\pi/2','\pi','3\pi/2','2pi'});
    xlim([0,2*pi]);
    ylabel('u(x,t)');
    ylim([min(u0)-padding,max(u0)+padding]);
    
    formatSpec = '/Figures/sine-squared_wave_%d.png';
    str = sprintf(formatSpec, times(i));
    exportgraphics(f, [pwd str], 'Resolution', 300);
    
    pause(0.05);
end